...
- `cloudflared.service`：Named Tunnel 的 `systemd` 单元。  
...
- `cloudflared-config.example.yml`：Named Tunnel 配置模板。  
...
- `cloudflared.service`: 待切换到 Named Tunnel  
  pending Named Tunnel cutover
...
- 固定域名：Named Tunnel 提供稳定 DNS 入口，适合 README、文档和手机访问。  
  Fixed hostnames: Named Tunnel provides stable DNS entrypoints suitable for docs, README links, and mobile access.
...
| 隧道 Tunnel | cloudflared | 通过 Named Tunnel 绑定固定子域名。 / Binds fixed subdomains through a Named Tunnel. |
...
- `cloudflared tunnel login` 生成的 `~/.cloudflared/cert.pem`。  
  `~/.cloudflared/cert.pem` generated by `cloudflared tunnel login`.

具备这两项后，就可以创建 Named Tunnel、route DNS，并把 `ai.example.com` / `drive.example.com` 替换成真实子域名。  
Once those are available, the server can create the Named Tunnel, route DNS, and replace `ai.example.com` / `drive.example.com` with real hostnames.
