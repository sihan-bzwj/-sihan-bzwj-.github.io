<div class="site-shell">
	<header class="topbar">
		<div class="brand">Sihan's Blog | 思涵的个人网站</div>
		<nav class="nav-pills" aria-label="Top navigation">
			<a class="pill" href="#posts">Blog | 文章</a>
			<a class="pill" href="cloud-drive/">云盘 | Cloud Drive</a>
			<a class="pill" href="https://ai.example.com/" target="_blank" rel="noreferrer noopener">AI | AI</a>
			<a class="social-link" href="https://github.com/sihan-bzwj/sihan-bzwj.github.io" target="_blank" rel="noreferrer noopener">GitHub</a>
		</nav>
	</header>

	<section class="hero">
		<div class="hero-panel">
			<span class="kicker">Home / Hub | 首页 / 入口</span>
			<h1 class="hero-title">云盘 / AI / 笔记入口<br>Personal Website Hub</h1>
			<p class="hero-lead">这里集中放笔记、云盘和 AI 入口，方便快速直达常用服务。 | A hub for notes, cloud drive, and AI entrypoints.</p>
			<div class="hero-meta">
				<span class="meta-chip">notes</span>
				<span class="meta-chip">cloud drive</span>
				<span class="meta-chip">ai</span>
			</div>
			<div class="hero-actions">
				<a class="button" href="#posts">浏览内容 | Browse</a>
				<a class="button-secondary" href="cloud-drive/">打开云盘 | Open Drive</a>
				<a class="button-secondary" href="https://ai.example.com/" target="_blank" rel="noreferrer noopener">打开 AI 站点 | AI Chat</a>
			</div>
		</div>

		<aside class="side-panel">
			<h2>站点说明 | About</h2>
			<p>这个首页只保留最常用的入口，后续内容会继续按需补充。 | Quick access to the main services, with more content added as needed.</p>
			<div class="card-actions">
				<a class="button-secondary" href="https://github.com/sihan-bzwj/sihan-bzwj.github.io" target="_blank" rel="noreferrer noopener">GitHub 源码 | Source Code</a>
			</div>
		</aside>
	</section>

	<section class="section" id="posts">
		<div class="section-header">
			<div>
				<h2>内容入口 | Contents</h2>
				<p>下面是几个常用入口，方便从主页直接跳到云盘、AI 和源码。 | Quick links to the cloud drive, AI, and source code.</p>
			</div>
		</div>

		<div class="post-list">
			<a class="list-card post-card" href="cloud-drive/">
				<div class="post-body">
					<div class="post-meta">Featured · 精选项目</div>
					<div class="post-title">云盘项目 | Cloud Drive</div>
					<p class="post-summary">远程文件存储和共享服务，支持大文件上传下载。| Remote file storage and sharing service with large file support.</p>
					<div class="tag-list">
						<span class="tag">storage</span>
						<span class="tag">files</span>
						<span class="tag">cloud</span>
					</div>
				</div>
				<div class="post-aside"><span>快速访问云盘 | Quick Access</span></div>
			</a>

			<a class="list-card post-card" href="https://ai.example.com/" target="_blank" rel="noreferrer noopener">
				<div class="post-body">
					<div class="post-meta">Featured · 精选项目</div>
					<div class="post-title">AI 对话平台 | AI Chat</div>
					<p class="post-summary">基于 LobeChat 的云端 AI 对话平台，支持 56+ 个大语言模型供应商。| LobeChat-based AI chat platform supporting 56+ LLM providers.</p>
					<div class="tag-list">
						<span class="tag">ai</span>
						<span class="tag">chat</span>
						<span class="tag">models</span>
					</div>
				</div>
				<div class="post-aside"><span>进入 AI 站点 | Enter AI</span></div>
			</a>

			<a class="list-card post-card" href="https://github.com/sihan-bzwj/sihan-bzwj.github.io" target="_blank" rel="noreferrer noopener">
				<div class="post-body">
					<div class="post-meta">Source · 源码库</div>
					<div class="post-title">项目源码 | Repository</div>
					<p class="post-summary">完整的项目源代码和部署配置，包括 AI 平台、云盘服务和文档。| Full source code with AI platform, cloud drive service, and documentation.</p>
					<div class="tag-list">
						<span class="tag">open source</span>
						<span class="tag">repository</span>
						<span class="tag">deployment</span>
					</div>
				</div>
				<div class="post-aside"><span>查看源码 | View Code</span></div>
			</a>
		</div>

		<footer class="site-footer" aria-label="网站统计">
			<div class="site-footer__item">
				<span class="site-footer__label">浏览人数统计 | Visitors</span>
				<strong class="site-footer__value" data-visitor-count>加载中</strong>
			</div>
			<div class="site-footer__item">
				<span class="site-footer__label">更新时间 | Updated</span>
				<strong class="site-footer__value">2026-04-11</strong>
			</div>
		</footer>

	</section>

</div>
