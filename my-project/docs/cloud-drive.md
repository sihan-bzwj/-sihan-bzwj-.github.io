<div class="site-shell">
  ...
  <section class="section" id="drive">
    <div class="section-header">
      <div>
        <h2>访问方式 | Access</h2>
        <p>云盘已经部署在独立域名上。为避免跨站 iframe 导致的加载变慢或访问失败，这里不再嵌入实时预览，直接使用上面的按钮打开即可。| The drive already runs on its own domain. To avoid slow loads or failed access caused by cross-site iframe embedding, this page no longer includes a live preview.</p>
      </div>
    </div>
